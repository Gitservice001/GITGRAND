...
const SOUND_PRESETS = {
    SHORT: { loudDistance: 30, maxDistance: 50 },
    MEDIUM: { loudDistance: 50, maxDistance: 70 },
    LONG: { loudDistance: 100, maxDistance: 120 },
    VEHICLE: { loudDistance: 5, maxDistance: 15 }
};
const DISTANCE_MULTIPLIER = 2; // Чтоб удалять звуки на большем расстоянии из soundsData, а не сразу как только вышел

let lastPlayerPosition = { x: 0, y: 0, z: 0, heading: 0 };
...
function createSound(soundUrl, position, options = {}) {
    if (!spatialSoundBrowser) return null;
    
    const id = options.id || `sound_${++soundIdCounter}`;
    
    if (soundsData.has(id)) removeSound(id);
    
    const soundData = { soundUrl, position, options };
...
function createVehicleSound(soundUrl, vehicle, preset = 'VEHICLE', customOptions = {}) {
    ...
    return createSoundWithPreset(soundUrl, { x: pos.x, y: pos.y, z: pos.z }, preset, {
        ...customOptions,
        vehicle,
        isVehicleSound: true
    });
...
function removeSound(id) {
    if (!spatialSoundBrowser || !soundsData.has(id)) return;
...
function updateListener() {
    if (!spatialSoundBrowser || !soundsData.size) return;
    ...
    const posChanged = lastPlayerPosition.x !== pos.x || 
                       lastPlayerPosition.y !== pos.y || 
                       lastPlayerPosition.z !== pos.z || 
                       lastPlayerPosition.heading !== heading;

    if (posChanged) {
        lastPlayerPosition = { x: pos.x, y: pos.y, z: pos.z, heading };
        spatialSoundBrowser.execute(`updateListener(${JSON.stringify({ x: pos.x, y: pos.y, z: pos.z })}, ${heading})`);
    }
...
function updateVehicleSounds() {
    if (!vehicleSounds.size) return;
    
    const operations = [];
    const toRemove = [];
    
    vehicleSounds.forEach((vehData, vehicle) => {
        const { id, totalDoors } = vehData;
        ...
        if (soundData) {
            soundData.position = { x: vehPos.x, y: vehPos.y, z: vehPos.z };
            operations.push({
                type: 'updatePosition',
                id: id,
    ...
    if (operations.length) spatialSoundBrowser.execute(`batchUpdate(${JSON.stringify(operations)})`);

}

function getVehicleDoorCount(vehicle) {
    if (!vehicle || !mp.vehicles.exists(vehicle)) return 0;
    
    let count = 0;
    for (let i = -1; i < 8; i++) {
        try {
            const doorPosition = vehicle.getEntryPositionOfDoor(i);
            if (doorPosition && (doorPosition.x || doorPosition.y || doorPosition.z)) count++;
...
function getVehicleOpenDoorsCount(vehicle, totalDoors) {
    ...
    for (let i = -1; i < Math.min(totalDoors, 8); i++) {
        if (vehicle.getDoorAngleRatio(i) > 0) openDoors++;
    }
    return openDoors;
}
...
mp.events.add('playerReady', () => {
    ...
    if (!updateInterval) updateInterval = setInterval(updateListener, 200);
});
...
function showSoundsInfo() {
    if (!soundsData.size) return mp.gui.chat.push('[Spatial] No active sounds');
...
function debugCreatePresetSound(preset) {
    ...
    const soundId = createSoundWithPreset(testUrl, { x: pos.x, y: pos.y, z: pos.z }, preset);
...
function debugCreateVehicleSound() {    
    if (!mp.players.local.vehicle) return mp.gui.chat.push('[Spatial] You must be in a vehicle!');
...
function debugOpenDoor(index) {
    if (!mp.vehicles.exists(debugVehicle)) return mp.gui.chat.push('[Spatial] Vehicle not found!');
    debugVehicle.setDoorOpen(index, false, true);
}

function registerDebugBinds() {
    mp.keys.bind(0x67, true, () => debugCreatePresetSound('SHORT'));   // Numpad 7
    mp.keys.bind(0x68, true, () => debugCreatePresetSound('MEDIUM'));  // Numpad 8
    mp.keys.bind(0x69, true, () => debugCreatePresetSound('LONG'));    // Numpad 9
    mp.keys.bind(0x66, true, () => debugCreateVehicleSound());         // Numpad 6
    mp.keys.bind(0x2E, true, () => {                                   // DELETE
        removeAllSounds();
        mp.gui.chat.push('[Spatial] All sounds removed');
    });
    mp.keys.bind(0x49, true, () => showSoundsInfo());                  // I

    mp.keys.bind(0x30, true, () => debugOpenDoor(0));
    mp.keys.bind(0x31, true, () => debugOpenDoor(1));
    mp.keys.bind(0x32, true, () => debugOpenDoor(2));
    mp.keys.bind(0x33, true, () => debugOpenDoor(3));
    mp.keys.bind(0x34, true, () => debugOpenDoor(4));
    mp.keys.bind(0x35, true, () => debugOpenDoor(5));
