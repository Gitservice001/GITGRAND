...
(function (global, factory) {
  ...
  function getPropDefaultValue(vm, prop, key) {
    ...
    if (typeof def === "function" && getType(prop.type) !== "Function") {
      return def.call(vm);
    } else {
      return def;
    }
  ...
  function extractPropsFromVNodeData(data, Ctor, tag) {
    ...
    if (isDef(attrs) || isDef(props)) {
      for (var key in propOptions) {
        ...
        if (!checkProp(res, props, key, altKey, true)) {
...
;
  ...
  function flushSchedulerQueue() {
    flushing = true;
    var watcher;
    var id; // Sort queue before flush.
    // This ensures that:
      ...
      if (!isObject(value)) {
        ...
        function loop(key) {
          if (key === "class" || key === "style" || isReservedAttribute(key)) {
            ...
            hash = asProp || config.mustUseProp(tag, type, key) ? data.domProps ||= {} : data.attrs ||= {};
          }
          if (!(key in hash)) {
            hash[key] = value[key];
            if (isSync) {
  ...
  function renderStatic(index, isInFor) {
    var cached = this._staticTrees ||= [];
    var tree = cached[index];
    // if has already-rendered static tree and not inside v-for,
    // we can reuse the same tree by doing a shallow clone.
    if (tree && !isInFor) {
      if (Array.isArray(tree)) {
        return cloneVNodes(tree);
      } else {
        return cloneVNode(tree);
      }
    }
  ...
  function mustUseProp(tag, type, attr) {
  ...
  function getXlinkProp(name) {
    if (isXlink(name)) {
      return name.slice(6, name.length);
    } else {
      return "";
    }
  }
  ...
  function createPatchFunction(backend) {
    ...
    function isUnknownElement$$1(vnode, inVPre) {
      return !inVPre && !vnode.ns && (!config.ignoredElements.length || !config.ignoredElements.some(function (ignore) {
        if (isRegExp(ignore)) {
          return ignore.test(vnode.tag);
        } else {
          return ignore === vnode.tag;
        }
    ...
    function updateChildren(parentElm, oldCh, newCh, insertedVnodeQueue, removeOnly) {
      ...
      var idxInOld;
      var vnodeToMove;
      var refElm; // removeOnly is a special flag used only by <transition-group>
  ...
  function locateNode(vnode) {
    if (vnode.componentInstance && (!vnode.data || !vnode.data.transition)) {
      return locateNode(vnode.componentInstance._vnode);
    } else {
      return vnode;
    }
  }
  ...
  var Transition = {
    ...
    render: function render(h) {
      ...
      var data = (child.data ||= {}).transition = extractTransitionData(this);
  ...
  function parseHTML(html, options) {
    ...
    while (html) {
      ...
      if (!lastTag || !isPlainTextElement(lastTag)) {
        ...
        var reStackedTag = reCache[stackedTag] ||= new RegExp("([\\s\\S]*?)(</" + stackedTag + "[^>]*>)", "i");
  ...
  function checkExpression(exp, text, errors) {
    try {
      ...
      if (keywordMatch) {
        ...
        errors.push("invalid expression: " + (e.Message, nLog.Type.Error) + " in\n\n    " + exp + "\n\n  Raw expression: " + text.trim() + "\n");
      }
    }
  }
