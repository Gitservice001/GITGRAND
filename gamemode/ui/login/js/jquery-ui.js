...
(function (factory) {
  ...
  $.widget = function (name, base, prototype) {
    ...
    var constructor;
    var basePrototype; // ProxiedPrototype allows the provided prototype to remain unmodified
      ...
      if (typeof key === "string") {
        ...
        if (parts.length) {
          ...
          if (arguments.length === 1) {
            if (curOption[key] === undefined) {
              return null;
            } else {
              return curOption[key];
            }
          }
          curOption[key] = value;
        } else {
    ...
    $.ui.position = {
      fit: {
        left: function (position, data) {
          ...
          var newOverRight; // Element is wider than within
        ...
        top: function (position, data) {
          ...
          var newOverBottom; // Element is taller than within
  ...
  var dataSpaceStyle = "ui-effects-style";
  var dataSpaceAnimated = "ui-effects-animated";
  var
  ...
  (function (jQuery, undefined) {
    ...
    color.fn = jQuery.extend(color.prototype, {
      parse: function (red, green, blue, alpha) {
        ...
        var rgba = this._rgba = []; // More than 1 argument specified - assume ( red, green, blue, alpha )
        if (green !== undefined) {
      ...
      transition: function (other, distance) {
        ...
        each(space.props, function (key, prop) {
          ...
          var type = propTypes[prop.type] || {}; // If null, don't override start value
          if (endValue === null) {
            return;
          }
    ...
    each(spaces, function (spaceName, space) {
      ...
      var cache = space.cache;
      var to = space.to;
      var from = space.from; // Makes rgba() and hsla()
  ...
  (function () {
    ...
    function getElementStyles(elem) {
      var key;
      var len;
      var style = elem.ownerDocument.defaultView ? elem.ownerDocument.defaultView.getComputedStyle(elem, null) : elem.currentStyle;
      var styles = {};
    ...
    $.effects.animateClass = function (value, duration, easing, callback) {
      ...
      return this.queue(function () {
        ...
        var allAnimations = o.children ? animated.find("*").addBack() : animated; // Map the animated objects to store the original styles.
  ...
  (function () {
    ...
    if ($.uiBackCompat !== false) {
      $.extend($.effects, {
        ...
        createWrapper: function (element) {
          ...
          var active = document.activeElement; // Support: Firefox
    ...
    $.extend($.effects, {
      ...
      scaledDimensions: function (element, percent, direction) {
        ...
        var y = direction !== "vertical" ? (percent || 100) / 100 : 1;
        return {
      ...
      createPlaceholder: function (element) {
        ...
        var position = element.position(); // Lock in margins first to account for form elements, which
      ...
      removePlaceholder: function (element) {
        ...
        var placeholder = element.data(dataKey);
        if (placeholder) {
          placeholder.remove();
    ...
    $.fn.extend({
      ...
      () {
        ...
        var defaultMode = effectMethod.mode;
        var queue = args.queue;
        var queueName = queue || "fx";
        var complete = args.complete;
        var mode = args.mode;
        var modes = [];
        function prefilter(next) {
          var el = $(this);
          var normalizedMode = $.effects.mode(el, mode) || defaultMode; // Sentinel for duck-punching the :animated psuedo-selector
        ...
        if (queue === false) {
          ...
          return this.queue(queueName, prefilter).queue(queueName, run);
        }
      },
      show: function (orig) {
      ...
      transfer: function (options, done) {
        ...
        var transfer = $("<div class='ui-effects-transfer'></div>").appendTo("body").addClass(options.className).css({
    ...
    $.extend(baseEasings, {
      ...
      Elastic: function (p) {
        if (p === 0 || p === 1) {
          ...
          return -Math.pow(2, (p - 1) * 8) * Math.sin(((p - 1) * 80 - 7.5) * Math.PI / 15);
        }
      },
      Back: function (p) {
        return p * p * (p * 3 - 2);
      ...
      Bounce: function (p) {
        ...
        return 1 / Math.pow(4, 3 - bounce) - Math.pow((pow2 * 3 - 2) / 22 - p, 2) * 7.5625;
      }
    });
  ...
  var effectsEffectBlind = $.effects.define("blind", "hide", function (options, done) {
    ...
    var start = element.cssClip();
    var animate = {
      clip: $.extend({}, start)
    };
    var placeholder = $.effects.createPlaceholder(element);
  ...
  var effectsEffectBounce = $.effects.define("bounce", function (options, done) {
    ...
    var speed = options.duration / anims;
    ...
    var i = 0;
    var queuelen = element.queue().length;
  ...
  var effectsEffectClip = $.effects.define("clip", "hide", function (options, done) {
    ...
    var both = direction === "both";
    var horizontal = both || direction === "horizontal";
    var vertical = both || direction === "vertical";
  ...
  var effectsEffectExplode = $.effects.define("explode", "hide", function (options, done) {
    ...
    var mx;
    var my;
    var rows = options.pieces ? Math.round(Math.sqrt(options.pieces)) : 3;
    var cells = rows;
    var element = $(this);
    var mode = options.mode;
    var show = mode === "show";
    ...
    var height = Math.ceil(element.outerHeight() / rows);
    var pieces = []; // Children animate complete:
  ...
  var effectsEffectFold = $.effects.define("fold", "hide", function (options, done) {
    ...
    var horizFirst = !!options.horizFirst;
    var ref = horizFirst ? ["right", "bottom"] : ["bottom", "right"];
    ...
    var distance = [start[ref[0]], start[ref[1]]];
    var queuelen = element.queue().length;
    if (percent) {
  ...
  var effectsEffectSize = $.effects.define("size", function (options, done) {
    ...
    // Set options
    mode = options.mode;
    var restore = mode !== "effect";
    var scale = options.scale || "both";
    var origin = options.origin || ["middle", "center"];
    var position = element.css("position");
    var pos = element.position();
    var original = $.effects.scaledDimensions(element);
    var from = options.from || original;
    var to = options.to || $.effects.scaledDimensions(element, 0);
    $.effects.createPlaceholder(element);
    if (mode === "show") {
      temp = from;
      from = to;
    ...
    if (scale === "content" || scale === "both") {
      ...
      element.find("*[width]").each(function () {
        var child = $(this);
        var childOriginal = $.effects.scaledDimensions(child);
        var childFrom = {
  ...
  var effectsEffectScale = $.effects.define("scale", function (options, done) {
    ...
    var el = $(this);
    var mode = options.mode;
    var percent = parseInt(options.percent, 10) || (parseInt(options.percent, 10) === 0 ? 0 : mode !== "effect" ? 0 : 100);
  ...
  var effectsEffectPulsate = $.effects.define("pulsate", "show", function (options, done) {
    ...
    var duration = options.duration / anims;
    var animateTo = 0;
    var i = 1;
    var queuelen = element.queue().length;
  ...
  var effectsEffectShake = $.effects.define("shake", function (options, done) {
    ...
    var anims = times * 2 + 1;
    var speed = Math.round(options.duration / anims);
    var ref = direction === "up" || direction === "down" ? "top" : "left";
  ...
  var effectsEffectSlide = $.effects.define("slide", "show", function (options, done) {
    ...
    var ref = direction === "up" || direction === "down" ? "top" : "left";
    var positiveMotion = direction === "up" || direction === "left";
    var distance = options.distance || element[ref === "top" ? "outerHeight" : "outerWidth"](true);
    var animation = {};
    $.effects.createPlaceholder(element);
  ...
  var labels = $.fn.labels = function () {
    ...
    var id;
    var labels;
    var ancestors; // Check control.labels first
  ...
  var widgetsAccordion = $.widget("ui.accordion", {
    ...
    _toggle: function (data) {
      ...
      var toHide = this.prevShow.length ? this.prevShow : data.oldPanel; // Handle activating a panel during the animation for another activation
  ...
  var widgetsMenu = $.widget("ui.menu", {
    ...
    _create: function () {
      ...
      this._on({
        ...
        "mouseenter .ui-menu-item": function (event) {
          ...
          var target = $(event.currentTarget); // Ignore bubbled events on parent items, see #11641
    ...
    _destroy: function () {
      ...
      var submenus = items.children(".ui-menu-item-wrapper").removeUniqueId().removeAttr("tabIndex role aria-haspopup"); // Destroy (sub)menus
    ...
    refresh: function () {
      ...
      newSubmenus = submenus.filter(":not(.ui-menu)").hide().attr({
        ...
        var menu = $(this);
        var item = menu.prev();
        var submenuCaret = $("<span>").data("ui-menu-submenu-caret", true);
    ...
    _filterMenuItems: function (character) {
      ...
      var regex = new RegExp("^" + escapedCharacter, "i");
  ...
  $.widget("ui.autocomplete", {
    ...
    _create: function () {
      ...
      var isInput = nodeName === "input"; // Textareas are always multi-line
      ...
      this._on(this.menu.element, {
        ...
        menuselect: function (event, ui) {
          ...
          var previous = this.previous; // Only trigger when focus was lost (click on menu)
          if (this.element[0] !== $.ui.safeActiveElement(this.document[0])) {
            this.element.trigger("focus");
    ...
    _searchTimeout: function (event) {
      ...
      this.searching = this._delay(function () {
        ...
        var menuVisible = this.menu.element.is(":visible");
        var modifierKey = event.altKey || event.ctrlKey || event.metaKey || event.shiftKey;
  ...
  var widgetsControlgroup = $.widget("ui.controlgroup", {
    ...
    _initWidgets: function () {
      var that = this;
      var childWidgets = []; // First we iterate over each of the items options
    ...
    _callChildMethod: function (method) {
      this.childWidgets.each(function () {
        var element = $(this);
        var data = element.data("ui-controlgroup-data");
        if (data && data[method]) {
          data[method]();
        }
      });
  ...
  $.widget("ui.checkboxradio", [$.ui.formResetMixin, {
    ...
    refresh: function () {
      var checked = this.element[0].checked;
      var isDisabled = this.element[0].disabled;
  ...
  $.widget("ui.button", {
    ...
    _updateIcon: function (option, value) {
      ...
      var position = icon ? this.options.iconPosition : value;
      var displayBlock = position === "top" || position === "bottom"; // Create icon
      if (!this.icon) {
        this.icon = $("<span>");
    ...
    _setOptions: function (options) {
      ...
      var newIcon = options.icon === undefined ? this.options.icon : options.icon;
  ...
  $.extend(Datepicker.prototype, {
    ...
    _optionDatepicker: function (target, name, value) {
      ...
      if (arguments.length === 2 && typeof name === "string") {
        if (name === "defaults") {
          return $.extend({}, $.datepicker._defaults);
        } else if (inst) {
          if (name === "all") {
            return $.extend({}, inst.settings);
          } else {
            return this._get(inst, name);
          }
        } else {
          return null;
        }
    ...
    _updateDatepicker: function (inst) {
      ...
      var width = 17;
      var activeCell = inst.dpDiv.find("." + this._dayOverClass + " a");
    ...
    _selectDate: function (id, dateStr) {
      ...
      var target = $(id);
      var inst = this._getInst(target[0]);
      dateStr = dateStr ?? this._formatDate(inst);
      if (inst.input) {
    ...
    _ticksTo1970: (718685 + Math.floor(1970 / 4) - Math.floor(1970 / 100) + Math.floor(1970 / 400)) * 24 * 60 * 60 * 10000000,
    ...
    _generateHTML: function (inst) {
      ...
      var today = this._daylightSavingAdjust(new Date(tempDate.getFullYear(), tempDate.getMonth(), tempDate.getDate()));
      var
      // clear time
  ...
  $.widget("ui.draggable", $.ui.mouse, {
    ...
    _blurActiveElement: function (event) {
      ...
      var target = $(event.target); // Don't blur if the event occurred on an element that is within
    ...
    _createHelper: function (event) {
      ...
      helper = helperIsFunction ?
      	...
      	( o.helper === "clone" ?
      		...
      		this.element );*/
      helper = helperIsFunction ? $(o.helper.apply(this.element[0], [event])) : o.helper === "clone" ? this.element.clone().removeAttr("id").appendTo("body") : this.element;
    ...
    _getParentOffset: function () {
      ...
      var document = this.document[0]; // This is a special case where we need to modify a offset calculated on start, since the
      // following happened:
    ...
    _generatePosition: function (event, constrainPosition) {
      ...
      var pageY = event.pageY; // Cache the scroll
  ...
  $.ui.plugin.add("draggable", "connectToSortable", {
    ...
    drag: function (event, ui, draggable) {
      $.each(draggable.sortables, function () {
        ...
        var sortable = this; // Copy over variables that sortable's _intersectsWith uses
  ...
  $.ui.plugin.add("draggable", "scroll", {
    ...
    drag: function (event, ui, i) {
      var o = i.options;
      var scrolled = false;
      var scrollParent = i.scrollParentNotHidden[0];
  ...
  $.widget("ui.resizable", $.ui.mouse, {
    ...
    _respectSize: function (data) {
      ...
      var dh = this.originalPosition.top + this.originalSize.height;
    ...
    _getPaddingPlusBorderDimensions: function (element) {
      var i = 0;
      var widths = [];
      var borders = [element.css("borderTopWidth"), element.css("borderRightWidth"), element.css("borderBottomWidth"), element.css("borderLeftWidth")];
      var paddings = [element.css("paddingTop"), element.css("paddingRight"), element.css("paddingBottom"), element.css("paddingLeft")];
      for (; i < 4; i++) {
  ...
  $.ui.plugin.add("resizable", "animate", {
    stop: function (event) {
      ...
      var soffseth = ista && that._hasScroll(pr[0], "left") ? 0 : that.sizeDiff.height;
      var soffsetw = ista ? 0 : that.sizeDiff.width;
      var style = {
        width: that.size.width - soffsetw,
        height: that.size.height - soffseth
      };
      var left = parseFloat(that.element.css("left")) + (that.position.left - that.originalPosition.left) || null;
      var top = parseFloat(that.element.css("top")) + (that.position.top - that.originalPosition.top) || null;
  ...
  $.ui.plugin.add("resizable", "alsoResize", {
    ...
    resize: function (event, ui) {
      ...
      $(o.alsoResize).each(function () {
        ...
        var style = {};
        var css = el.parents(ui.originalElement[0]).length ? ["width", "height"] : ["width", "height", "top", "left"];
  ...
  $.widget("ui.dialog", {
    ...
    _createButtons: function () {
      var that = this;
      var buttons = this.options.buttons; // If we already have a button pane, remove it
    ...
    _makeResizable: function () {
      ...
      this.uiDialog.resizable({
        ...
        stop: function (event, ui) {
          ...
          var left = offset.left - that.document.scrollLeft();
          var top = offset.top - that.document.scrollTop();
    ...
    _untrackInstance: function () {
      ...
      var exists = $.inArray(this, instances);
      if (exists !== -1) {
    ...
    _size: function () {
      ...
      var options = this.options; // Reset content sizing
  ...
  $.widget("ui.droppable", {
    ...
    _drop: function (event, custom) {
      ...
      var childrenIntersection = false; // Bail if draggable and droppable are same element
      if (!draggable || (draggable.currentItem || draggable.element)[0] === this.element[0]) {
        return false;
      }
  ...
  var intersect = $.ui.intersect = function () {
    ...
    return function (draggable, droppable, toleranceMode, event) {
      ...
      var y1 = (draggable.positionAbs || draggable.position.absolute).top + draggable.margins.top;
  ...
  */
  ...
  var widgetsProgressbar = $.widget("ui.progressbar", {
    ...
    _constrainedValue: function (newValue = this.options.value) {
    ...
    _percentage: function () {
      if (this.indeterminate) {
        ...
        return (this.options.value - this.min) * 100 / (this.options.max - this.min);
      }
    },
  ...
  var widgetsSelectable = $.widget("ui.selectable", $.ui.mouse, {
    ...
    _create: function () {
      ...
      this.refresh = function () {
        ...
        that.selectees.each(function () {
          var $this = $(this);
          var selecteeOffset = $this.offset();
          var pos = {
    ...
    _mouseDrag: function (event) {
      ...
      this.selectees.each(function () {
        var selectee = $.data(this, "selectable-item");
        var hit = false;
        var offset = {}; //prevent helper from being selected if appendTo: selectable
  ...
  var widgetsSelectmenu = $.widget("ui.selectmenu", [$.ui.formResetMixin, {
    ...
    _drawButton: function () {
      var icon;
      var that = this;
      var item = this._parseOption(this.element.find("option:selected"), this.element[0].selectedIndex); // Associate existing label with the new button
  ...
  var widgetsSortable = $.widget("ui.sortable", $.ui.mouse, {
    ...
    _mouseDrag: function (event) {
      ...
      var intersection;
      var o = this.options;
      var scrolled = false; //Compute the helpers position
    ...
    _intersectsWithSides: function (item) {
      ...
      var isOverRightHalf = this._isOverAxis(this.positionAbs.left + this.offset.click.left, item.left + item.width / 2, item.width);
    ...
    _connectWith: function () {
      var options = this.options;
      if (options.connectWith.constructor === String) {
        return [options.connectWith];
      } else {
        return options.connectWith;
      }
    },
    ...
    _refreshItems: function (event) {
      ...
      var connectWith = this._connectWith(); //Shouldn't be run the first time through due to massive slow-down
    ...
    _contactContainers: function (event) {
      ...
      var innermostIndex = null; // Get innermost container that intersects with item
      for (i = this.containers.length - 1; i >= 0; i--) {
    ...
    _createHelper: function (event) {
      var o = this.options;
      var helper = $.isFunction(o.helper) ? $(o.helper.apply(this.element[0], [event, this.currentItem])) : o.helper === "clone" ? this.currentItem.clone() : this.currentItem; //Add the helper to the DOM if that didn't happen already
      if (!helper.parents("body").length) {
    ...
    _convertPositionTo: function (d, pos) {
      ...
      var mod = d === "absolute" ? 1 : -1;
      var scroll = this.cssPosition === "absolute" && (this.scrollParent[0] === this.document[0] || !$.contains(this.scrollParent[0], this.offsetParent[0])) ? this.offsetParent : this.scrollParent;
    ...
    _generatePosition: function (event) {
      ...
      var scroll = this.cssPosition === "absolute" && (this.scrollParent[0] === this.document[0] || !$.contains(this.scrollParent[0], this.offsetParent[0])) ? this.offsetParent : this.scrollParent;
      var scrollIsRootNode = /(html|body)/i.test(scroll[0].tagName); // This is another very weird special case that only happens for relative elements:
  ...
  $.widget("ui.spinner", {
    ...
    _increment: function (i) {
      ...
      if (incremental) {
        if ($.isFunction(incremental)) {
          return incremental(i);
        } else {
          return Math.floor(i * i * i / 50000 - i * i / 500 + i * 17 / 200 + 1);
        }
      }
      return 1;
    },
    ...
    _adjustValue: function (value) {
      var base;
      var aboveMin;
      var options = this.options; // Make sure we're at a valid step
    ...
    _format: function (value) {
      ...
      if (window.Globalize && this.options.numberFormat) {
        return Globalize.format(value, this.options.numberFormat, this.options.culture);
      } else {
        return value;
      }
    },
  ...
  $.widget("ui.tabs", {
    ...
    refresh: function () {
      var options = this.options;
      var lis = this.tablist.children(":has(a[href])"); // Get disabled tabs from class attribute from HTML
    ...
    _processTabs: function () {
      ...
      this.anchors.each(function (i, anchor) {
        ...
        var originalAriaControls = tab.attr("aria-controls"); // Inline tab
    ...
    _activate: function (index) {
      var anchor;
      var active = this._findActive(index); // Trying to activate the already active panel
  ...
  $.widget("ui.tooltip", {
    ...
    close: function (event) {
      ...
      var tooltipData = this._find(target); // The tooltip may already be closed
      if (!tooltipData) {
    ...
    _tooltip: function (element) {
      ...
      var content = $("<div>").appendTo(tooltip);
      var id = tooltip.uniqueId().attr("id");
    ...
    _destroy: function () {
      ...
      $.each(this.tooltips, function (id, tooltipData) {
        ...
        var event = $.Event("blur");
        var element = tooltipData.element;
